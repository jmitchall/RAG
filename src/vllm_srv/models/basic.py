...
def main():
    ...
    # llm = LLM(model="facebook/opt-125m")  # Only 125 million parameters
    ...
    llm = LLM(
        ...
        gpu_memory_utilization=0.75,  # Use 75% of graphics card memory (leaves room for cache)
        max_model_len=2048,  # Handle conversations up to 2048 words (saves memory)
        ...
        kv_cache_dtype="fp8"  # Use 8-bit precision for cache (saves memory)
    )

    # OPTION 3: High quality model (requires HuggingFace account and login)
    # llm = LLM(
    #     model="mistralai/Mistral-7B-Instruct-v0.2",  # 7 billion parameters
    #     gpu_memory_utilization=0.85,                  # Use 85% of graphics card memory  
    #     max_model_len=4096,                           # Handle conversations up to 4096 words
    #     dtype="float16"                               # Use half-precision numbers
    # )

    # OPTION 4: Extremely powerful model (needs 20+ GB graphics card memory)
    # llm = LLM(
    #     model="mistralai/Mixtral-8x7B-Instruct-v0.1",  # 8 experts x 7B = 56B parameters!
    #     gpu_memory_utilization=0.85,                    # Use 85% of graphics card memory
    #     max_model_len=2048,                             # Shorter conversations to save memory
    #     dtype="float16",                                # Use half-precision numbers
    #     kv_cache_dtype="fp8"                            # Extra memory optimization
    # )
