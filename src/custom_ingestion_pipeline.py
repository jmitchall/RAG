...
def _attempt_progressive_truncation(
    ...
    """
    Attempt to generate embedding using progressively smaller truncations.
    
    Tries truncating the chunk to 7, 5, 3, 2, and 1 words in sequence until
    successful or all attempts are exhausted.
    ...
    """
    words = chunk.split()
    truncation_levels = [7, 5, 3, 2, 1]
    
    for max_words in truncation_levels:
        if len(words) < max_words:
            continue
            
        truncated_chunk = ' '.join(words[:max_words])
        try:
            ...
            if retry_count <= 10:
                print(f"   🆘 Emergency truncation succeeded for chunk {chunk_index} "
                      f"(truncated to {max_words} words)")
            elif retry_count == 11:
    ...
    for i, chunk in enumerate(chunk_texts):
        try:
            ...
            embedding = _attempt_progressive_truncation(
                embedding_manager, chunk, chunk_index, truncation_count
            )
...
def get_embedded_document_chunks_from_path(document_path :str, embedding_manager: EmbeddingManager, max_tokens: int, use_llama_index_chunking: bool = False) -> Tuple[List[np.ndarray], List[Document]]:
    """
    ...
    """
    if use_llama_index_chunking:
        chunk_texts, document_chunks  = load_llamaIndex_texts( document_path, max_tokens)
    else:
        chunk_texts, document_chunks  = load_langchain_texts( document_path, max_tokens)
...
def persist_vector_database(collection_name:str, vector_db_persisted_path:str, embedding_manager: EmbeddingManager,
                            max_tokens:int,  embedding_manager_dim:int, use_llama_index_chunking: bool ,
    ...
    chunked_embeddings, document_chunks = get_embedded_document_chunks_from_path(
        ...
        use_llama_index_chunking=use_llama_index_chunking   
    )
...
if __name__ == "__main__":
    ...
    db_type ='chroma'   # Change to "qdrant", "faiss", "chroma"
    ...
    for collection_name in document_collections:
        ...
        vector_db_persisted_path =f"{root_path}/vector_db_{collection_name}_{DATABASE_TYPE}"
